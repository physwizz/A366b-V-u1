...
static int s2mf301_fled_set_mode(struct s2mf301_fled_data *fled, int chan, int mode)
{
	...
	pr_info("%s: get gpio status as gpio_torch(%d) = %d, gpio_flash(%d) = %d.\n", __func__,gpio_torch,gpio_get_value(gpio_torch),gpio_flash,gpio_get_value(gpio_flash));
...
EXPORT_SYMBOL(s2mf301_rear_flash_store);
...
EXPORT_SYMBOL(s2mf301_rear_flash_show);
...
static int s2mf301_led_probe(struct platform_device *pdev)
{
	...
	pr_info("%s: end, int get flash_gpio(%d) = %d, get torch_gpio(%d) = %d \n", __func__,g_fled_data->flash_gpio,gpio_get_value(g_fled_data->flash_gpio),g_fled_data->torch_gpio,gpio_get_value(g_fled_data->torch_gpio));
	return 0;
}
