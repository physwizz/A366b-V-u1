...
static int process_received_msg(struct sk_buff *skb, struct nlmsghdr *nlh)
{
	...
	if(first_boot == 0) {
		first_boot = 1;
		cam_ois_cmd_init_work(data);
		cam_ois_reg_read_init_work(data);
		cam_ois_factory_mode_init_work(data);
	}
#endif
		return 0;
	}
