...
#else /* CONFIG_SUPPORT_DDI_COPR_FOR_LIGHT_SENSOR */
void light_decode_ac_short_fifo_data(int32_t *input, uint16_t *output)
{
	for (int i = 0; i < 32; i++) {
		output[2 * i] = (input[i] >> 16) & 0xFFFF;
		output[2 * i + 1] = input[i] & 0xFFFF;
	}
}

static ssize_t light_ac_short_fifo_show(struct device *dev,
	struct device_attribute *attr, char *buf)
{
	struct adsp_data *data = dev_get_drvdata(dev);
	uint16_t ac_short_fifo[64];
	uint16_t light_idx = get_light_sidx(data);
	uint16_t i = 0, j = 0;
	uint8_t cnt = 0;
	...
	light_decode_ac_short_fifo_data(data->msg_buf[light_idx], ac_short_fifo);

	mutex_unlock(&data->light_factory_mutex);

	i += snprintf(buf + i, PAGE_SIZE - i, "%d,%d,%d,",
				ac_short_fifo[0], ac_short_fifo[1], ac_short_fifo[2]);

	for (j = 3; j < 63; j++) {
		if (j == 62)
			i += snprintf(buf + i, PAGE_SIZE - i, "%x\n", ac_short_fifo[j]);
		else
			i += snprintf(buf + i, PAGE_SIZE - i, "%x,", ac_short_fifo[j]);
	}
...
#else
static DEVICE_ATTR(fifo_data, 0444, light_ac_short_fifo_show, NULL);
#endif
