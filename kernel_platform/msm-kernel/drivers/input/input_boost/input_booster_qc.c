...
void ib_set_booster(long *qos_values)
{
	...
	for (res_type = 0; res_type < allowed_res_count; res_type++) {
		...
		case HMPBOOST:
			...
			break;
		case LPMBIAS:
			// TODO:hyst_qos hyst_update_request(&lpm_bias_pm_qos_request, PM_QOS_MIN_LIMIT, value*NSEC_PER_MSEC);
...
//Bus boost factors initailized
	...
	/*
	if (!register_ddr) {
		path_touch_bw = of_icc_get(dev, "input_booster_ddr");
...
*/
	fill_bus_vector();
	// TODO:hyst_qos hyst_add_request(&lpm_bias_pm_qos_request, 0, "Input Booster");
	return 1;
}
...
*/
	...
	// TODO:hyst_qos hyst_remove_request(&lpm_bias_pm_qos_request);
}
