			...
			struct kobj_attribute *attr, char *buf)
{
	...
	len += snprintf(buf + len, MAX_BUF_SIZE, "  virt   |  gold   silver  |  gold   silver\n");
	for (i = 0; i < param.freq_count; i++) {
...
static void cflm_parse_dt(struct platform_device *pdev)
{
	...
	pr_info("%s: param: g_fmin_up(%d), ltl_max_freq(%d), big_min_freq(%d)\n", __func__,
						param.g_fmin_up, param.ltl_max_freq, param.big_min_freq);
};
#endif
