...
static int check_authic_supported(struct device_node *np, struct sec_auth_ds28e30_platform_data *pdata)
{
	...
	if (eur_det_gpio < 0) {
		...
		if (!IS_ERR_OR_NULL(pdata->p)) {
			state = pinctrl_lookup_state(pdata->p, "pulldown");
			if (IS_ERR_OR_NULL(state)) {
				auth_err("could not find pull down state (%li)\n", PTR_ERR(state));
			} else {
				ret = pinctrl_select_state(pdata->p, state);
				if (ret)
					auth_err("%s: Pinctrl state selection fail %d\n", __func__, ret);
			}
		} else {
			auth_err("No valid pinctrl state\n");
		}
#endif
		goto AUTH_IC_DEVICE;
	}
	...
	struct sec_auth_ds28e30_platform_data *pdata)
{
	...
	if (np == NULL) {
		...
		if (IS_ERR_OR_NULL(pdata->p)) {
			auth_err("Failed to get pinctrl\n");
		}
