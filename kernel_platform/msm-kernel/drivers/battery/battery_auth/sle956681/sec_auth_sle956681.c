...
static int check_authic_supported(struct device_node *np, struct authon_driver_platform_data *pdata)
{
	...
	if (eur_det_gpio < 0) {
		...
		if (!IS_ERR_OR_NULL(pdata->p)) {
			state = pinctrl_lookup_state(pdata->p, "pull_down");
			if (IS_ERR_OR_NULL(state)) {
				pr_err("could not find pull down state (%li)\n", PTR_ERR(state));
			} else {
				ret = pinctrl_select_state(pdata->p, state);
				if (ret)
					pr_err("%s: Pinctrl state selection fail %d\n", __func__, ret);
			}
		} else {
			pr_err("No valid pinctrl state\n");
		}
