...
static int setup_direct_charging_work_config(struct sm_dc_info *sm_dc)
{
	...
	if (sm_dc->i2c_sub)
		sm_dc->ops->set_charging_config(sm_dc->i2c_sub,
					sm_dc->wq.cv_gl - SM_DC_DUAL_CV_SUB_OFFS, sm_dc->wq.ci_gl, sm_dc->wq.cc_gl);

	return 0;
}

static int setup_dc_dual_work_config(struct sm_dc_info *sm_dc, u8 next_state)
{
	if (sm_dc->i2c_sub) {
		if (next_state == SM_DC_PRE_CC) {
			sm_dc->ops->set_dc_dual_cfg(sm_dc->i2c,
							sm_dc->wq.cv_gl, SM_DC_IBUSUCP_ON);
			sm_dc->ops->set_dc_dual_cfg(sm_dc->i2c_sub,
							sm_dc->wq.cv_gl - SM_DC_DUAL_CV_SUB_OFFS, SM_DC_IBUSUCP_OFF);
			pr_info("%s %s: VBATREG use sub DC\n", sm_dc->name, __func__);
		} else if (next_state == SM_DC_CC || next_state == SM_DC_CV) {
			sm_dc->ops->set_dc_dual_cfg(sm_dc->i2c,
							sm_dc->wq.cv_gl, SM_DC_IBUSUCP_ON);
			sm_dc->ops->set_dc_dual_cfg(sm_dc->i2c_sub,
							sm_dc->wq.cv_gl + SM_DC_DUAL_CV_OFFSET, SM_DC_IBUSUCP_OFF);
			pr_info("%s %s: VBATREG use main DC\n", sm_dc->name, __func__);
		}
	}
...
static int check_error_state(struct sm_dc_info *sm_dc, u8 retry_state)
{
	...
	if (sm_dc->i2c_sub) {
		if (sub_err == SM_DC_ERR_VBATREG || sub_err == SM_DC_ERR_IBUSREG) {
			sm_dc->err = sub_err;
			return sm_dc->err;
		}
...
static void pd_preset_dc_work(struct work_struct *work)
{
	...
	setup_dc_dual_work_config(sm_dc, SM_DC_PRE_CC);
...
static void pd_pre_cc_work(struct work_struct *work)
{
	...
	case LOOP_IBUSREG:
		...
		if (sm_dc->ta.v - (PPS_V_STEP * 2) >= sm_dc->config.ta_min_voltage) {
			...
			if (sm_dc->i2c_sub)
				sm_dc->ops->set_adc_mode(sm_dc->i2c_sub, SM_DC_ADC_MODE_CONTINUOUS);
			setup_dc_dual_work_config(sm_dc, SM_DC_CC);
	...
	if (sm_dc->wq.pps_cl) {
		...
		if (sm_dc->config.vbus_ctrl_mode) {
			sm_dc->ta.v += (adc_vbat > PRE_CC_RAMP_UP_LEV) ? PPS_V_STEP : (PPS_V_STEP * 2);
#if defined(CONFIG_SEC_FACTORY)
			sm_dc->ta.v += PPS_V_STEP * 2;
#endif
			if (sm_dc->ta.v > MIN(sm_dc->ta.v_max, sm_dc->config.dc_vbus_ovp_th - 350)) {
...
static void pd_cv_work(struct work_struct *work)
{
	...
	if ((sm_dc->wq.cv_cnt == 0) && (loop_status & (LOOP_VBATREG | LOOP_IBUSREG))) {
		...
		sm_dc->wq.cv_cnt = 2;
		setup_dc_dual_work_config(sm_dc, SM_DC_CV);
	}
	...
	if ((sm_dc->target_vbat == sm_dc->config.chg_float_voltage) &&
		(adc_ibus < CV_ST_SUB_DC_OFF_IBUS)) {
		if (sm_dc->i2c_sub) {
			if (sm_dc->ops->get_charging_enable(sm_dc->i2c_sub)) {
				sm_dc->ops->set_charging_enable(sm_dc->i2c_sub, 0);
