...
static void sm5461_init_reg_param(struct sm5461_charger *sm5461)
{
	...
	sm5461_write_reg(sm5461, SM5461_REG_CNTL3, 0xB3); /* RLTVOVP = 1.5x, RLTVUVP = 1.01x */
	...
	if (sm5461->pdata->single_mode)
		sm5461_write_reg(sm5461, SM5461_REG_IBATOCP_DG, 0x88); /* Deglitch for IBATOCP & IBUSOCP = 8ms */
	else
		sm5461_write_reg(sm5461, SM5461_REG_IBATOCP_DG, 0xA8); /* Deglitch for IBATOCP & IBUSOCP & IBUSUCP = 8ms */
...
static int psy_chg_get_health(struct sm5461_charger *sm5461)
{
	...
	if (state == SM_DC_ERR) {
		...
		dev_info(sm5461->dev, "%s: chg_on=%d, health=%d\n",
...
static int psy_chg_set_const_chg_voltage_max(struct sm5461_charger *sm5461, int max_vbat)
{
	...
	dev_info(sm5461->dev, "%s: max_vbat [%dmV] to [%dmV]\n",
...
static int sm5461_dc_set_charging_config(struct i2c_client *i2c, u32 cv_gl, u32 ci_gl, u32 cc_gl)
{
	...
	if (sm5461->chip_id == SM5461_SUB) {
		if (ci_gl <= SM5461_SIOP_LEV1)
			ibuslim = (ci_gl / 2) + (sm5461->pdata->ci_offset / 2);
		else if (ci_gl <= SM5461_SIOP_LEV2)
			ibuslim = (ci_gl / 2) + sm5461->pdata->ci_offset;
		else
			ibuslim = (ci_gl / 2) + (sm5461->pdata->ci_offset * 3 / 2);
	} else {
...
static u32 sm5461_get_dc_error_status(struct i2c_client *i2c)
{
	...
	if ((op_mode == OP_MODE_INIT) && (sm_dc->wq.retry_cnt < 3) && (sm_dc->chip_id != SM5461_SUB)) {
		...
		if ((err & (SM_DC_ERR_UNKNOWN | SM_DC_ERR_VBUSUVLO)) && sm_dc->chip_id == SM5461_SUB)
...
static int sm5461_dc_set_dual_cfg(struct i2c_client *i2c, u32 cv_gl, u8 en_ucp)
{
	struct sm5461_charger *sm5461 = i2c_get_clientdata(i2c);
	struct sm_dc_info *sm_dc = select_sm_dc_info(sm5461);

	/* vbatreg setting for dual DC */
	sm5461_set_vbatreg(sm5461, cv_gl);

	/* IBUSUCP on off */
	sm5461_update_reg(sm5461, SM5461_REG_IBUS_PROT, en_ucp, 0x1, 7);

	pr_info("%s %s: vbat_reg=%dmV, en_ibusucp=%d\n", sm_dc->name,
		__func__, cv_gl, en_ucp);
...
static const struct sm_dc_ops sm5461_dc_pps_ops = {
	...
	.set_dc_dual_cfg        = sm5461_dc_set_dual_cfg,
};
