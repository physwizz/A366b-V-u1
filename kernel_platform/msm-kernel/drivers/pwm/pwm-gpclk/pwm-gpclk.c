...
static int pwm_gpclk_get_state(struct pwm_chip *chip, struct pwm_device *pwm,
...
#endif
#if LINUX_VERSION_CODE >= KERNEL_VERSION(6, 0, 0)
static int pwm_gpclk_apply(struct pwm_chip *chip, struct pwm_device *pwm,
							const struct pwm_state *state)
{
	...
	if (state->period != pwm->state.period ||
		...
		err = pwm_gpclk_config(chip, pwm, state->duty_cycle, state->period);
		if(err)
			return err;
	...
	if (state->enabled != pwm->state.enabled) {
			if (state->enabled) {
				err = pwm_gpclk_enable(chip, pwm);
				if (err)
					return err;
			} else {
				pwm_gpclk_disable(chip, pwm);
				pwm->state.duty_cycle =  state->period >> 1;
				pwm->state.period = state->period;
			}
...
static int pwm_gpclk_probe(struct platform_device *pdev)
{
	...
	if (!virt_mmss_gp1_base) {
		pr_err("pwm_gpclk: Unable to ioremap MSM_MMSS_GP1 memory!");
		return -ENODEV;
	}
...
#else
	gc->gpclk_class = class_create("gpclk");
#endif
	...
	*/
	ret = sysfs_create_group(&gc->gpclk_dev->kobj, &gpclk_attr_group);
	if (ret) {
