...
from ELF import ELF
from ELF import DEFAULT_ARM_INST_WIDTH
...
class IntegrityRoutine(ELF):
    ...
    def get_pac_scs_gaps(self, section):
        """
        Find the PAC opcodes which will be replaced by SCS (Shadow Call Stack support)
        at runtime. The instructions are considered as gaps.
        :param section: section class instance where the search will be performed.
        :return: list of addresses to be excluded from the Integrity Check.
        """
        opcode_format = "<L"

        # 0x3f2303d5 opcode of 'paciasp' instruction
        pac_prolog_opcode = unpack(opcode_format, b'\x3f\x23\x03\xd5')
        # 0xbf2303d5 opcode of 'autiasp' instruction
        pac_epilog_opcode = unpack(opcode_format, b'\xbf\x23\x03\xd5')
        gap_addr_lst = []

        for symbol in section:
            symbol_content = self.get_data_by_vaddr(symbol.addr, symbol.size)

            try:
                opcode_iter = iter_unpack(opcode_format, symbol_content)
            except Exception as exc:
                raise Exception("Error: content of {symbol.name} is wrong.") from exc

            for record_idx, op_item in enumerate(opcode_iter):
                if op_item in [pac_prolog_opcode, pac_epilog_opcode] :
                    instr_base = symbol.addr + record_idx * DEFAULT_ARM_INST_WIDTH
                    gap_addr_lst.extend(range(instr_base, instr_base + DEFAULT_ARM_INST_WIDTH))
    ...
    def make_integrity(self, sec_sym, module_name, debug=False, print_reloc_gaps=False):
        ...
        if self.utils.is_config_enabled("CONFIG_UNWIND_PATCH_PAC_INTO_SCS"):
            gaps_patch_pac_to_scs_addrs = self.get_pac_scs_gaps(sec_sym[0])
        else:
            gaps_patch_pac_to_scs_addrs = []
        ...
        if debug:
            ...
            print("Number of runtime-patched PAC instructions in the module      : {}.".
                format(len(gaps_patch_pac_to_scs_addrs) / DEFAULT_ARM_INST_WIDTH))
        ...
        exclude_addrs.extend(gaps_patch_pac_to_scs_addrs)
