...
class Utils:
    ...
    @staticmethod
    def is_config_enabled(config_name):
        """
        Check if the kernel build config is enabled.
        :param config_name: name of the config (ex. CONFIG_CRYPTO_SKC_FIPS)
        :return: True if the requested config is active, False otherwise.
        """
        auto_conf_file_name = r"include/config/auto.conf"
        with open(auto_conf_file_name, "rb") as config_file:
            s = mmap.mmap(config_file.fileno(), 0, access=mmap.ACCESS_READ)
            return s.find(bytes(f"{config_name}=y", "utf-8")) != -1
