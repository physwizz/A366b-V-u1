...
int dsi_host_transfer_sub(struct mipi_dsi_host *host, struct dsi_cmd_desc *cmd)
{
	...
	if (cmd->ctrl_flags & DSI_CTRL_CMD_BROADCAST) {
		...
		if (((cmd->ctrl_flags & DSI_CTRL_CMD_READ) && rc <= 0) ||
			...
			LCD_ERR(vdd, "[%s] cmd transfer failed, rc=%d, cmd_flags=%x\n",
				   ss_get_cmd_name(vdd->cmd_type), rc, cmd->ctrl_flags);
			rc = -EINVAL;
		}
#else
		if (rc)
...
#if IS_ENABLED(CONFIG_DISPLAY_SAMSUNG)
	if (num_clk < 1)
		DSI_ERR("[SDE] ^^^^^^^^^^ No dsi clock, Check panel_common.MODEL.dtsi!\n");
#endif
...
int dsi_display_prepare(struct dsi_display *display)
{
	...
	if (!is_skip_op_required(display)) {
		...
		dsi_panel_set_pinctrl_state(display->panel, true); // JUN_TEMP
	}
#endif
	goto error;
