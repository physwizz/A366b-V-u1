...
#include "codecs/tfa9878/inc/tfa_ext.h"
#endif
#include "machine_defect_detector.h"

static u32 defer_count;
...
static bool is_defer_count_expired(u32 max_defer_count)
{
...
void check_snd_component(struct snd_soc_card *card, u32 max_defer_count)
{
...
static void send_abc_event(void)
{
#if IS_ENABLED(CONFIG_SEC_ABC)
...
static void report_tas_amp_i2c_fail(uint32_t i2caddr)
{
	pr_info("%s(%x)\n", __func__, i2caddr);
	send_abc_event();
}
...
static int report_tfa_amp_i2c_fail(int addr, int err, int rw, int cnt)
{
	pr_info("%s(%x)\n", __func__, addr);
	send_abc_event();
	return 0;
...
{
#if defined(CONFIG_SND_SOC_TAS25XX)
	tas25xx_register_i2c_error_callback(report_tas_amp_i2c_fail);
#elif defined(CONFIG_SND_SOC_TFA9878)
	tfa_i2c_err_register(report_tfa_amp_i2c_fail);
#else
	pr_info("%s: i2c err callback will not be registered.\n", __func__);
#endif
