...
PRODUCT_COPY_FILES += \
    $(CONFIG_HAL_SRC_DIR)/audconf/$(PROJECT_NAME)/$(TARGET_PRODUCT)/mixer_usb_gray.xml:$(TARGET_COPY_OUT_VENDOR)/etc/mixer_usb_gray.xml \
    $(CONFIG_HAL_SRC_DIR)/audconf/$(PROJECT_NAME)/$(TARGET_PRODUCT)/mixer_usb_default.xml:$(TARGET_COPY_OUT_VENDOR)/etc/mixer_usb_default.xml

# include audio rc file
PRODUCT_COPY_FILES += \
    $(CONFIG_HAL_SRC_DIR)/init.audio.samsung.rc:$(TARGET_COPY_OUT_VENDOR)/etc/init/init.audio.samsung.rc
...
# Increase AF ready frame count for TV voip noise issue
PRODUCT_PROPERTY_OVERRIDES += \
   ro.af.fcmultiplier = 2
