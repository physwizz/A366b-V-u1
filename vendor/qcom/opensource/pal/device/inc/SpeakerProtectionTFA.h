...
#define TFADSP_LIVEDATA_MEMTRACK_MAX_SIZE 640
...
private :
    ...
    uint8_t memtrack_buf[TFADSP_LIVEDATA_MEMTRACK_MAX_SIZE];
#endif
...
public:
    ...
    int32_t tfa_dsp_get_param(uint8_t *buffer, int buf_len, uint32_t param_id);
    ...
    uint32_t get_dummy_cal(uint32_t dev_idx);
#ifdef SET_IPCID_MIXER_CTL
