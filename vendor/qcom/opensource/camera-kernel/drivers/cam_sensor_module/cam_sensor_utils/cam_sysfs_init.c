...
static ssize_t rear2_dual_cal_show(struct device *dev,
	struct device_attribute *attr, char *buf)
{
	int copy_size = FROM_REAR_DUAL_CAL_SIZE;

	memcpy(buf, rear2_dual_cal, copy_size);
	return copy_size;
}
	...
	struct device_attribute *attr, const char *buf, size_t size)
{
	...
	memcpy(rear2_dual_cal, buf, copy_size);
			...
			size_t count)
{
	...
	aw36518_store(buf);
#elif defined(CONFIG_LEDS_S2MF301_FLASH)
	s2mf301_rear_flash_store(dev, attr, buf, count);
#endif
	return count;
}
...
#elif defined(CONFIG_LEDS_S2MF301_FLASH)
	return s2mf301_rear_flash_show(dev, attr, buf);
#else
	return 0;
#endif
}
...
static DEVICE_ATTR(rear2_dualcal, S_IRUGO|S_IWUSR|S_IWGRP, rear2_dual_cal_show, rear2_dual_cal_store);
