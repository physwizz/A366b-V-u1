			...
			struct device_attribute *attr, char *buf)
{
	...
	pr_info("[PDBG]gyro_calibration_show sysfs");
...
{
	pr_info("[PDBG]gyro_selftest_show sysfs");
	int rc = 0;
...
static ssize_t gyro_rawdata_test_store(struct device *dev,
	struct device_attribute *attr, const char *buf, size_t size)
{
	pr_info("[PDBG] gyro_rawdata_test_store sysfs");
	...
	if (ois_power) {
		if (size > MAX_EFS_DATA_LENGTH || size == 0) {
			pr_err("%s count is abnormal, count = %ld", __func__, size);
			return 0;
		}
...
static ssize_t gyro_rawdata_test_show(struct device *dev,
	struct device_attribute *attr, char *buf)
{
	pr_info("[PDBG] gyro_rawdata_test_show sysfs");
	int rc = 0;
	long raw_data_x = 0, raw_data_y = 0, raw_data_z = 0;
...
static ssize_t ois_hall_position_show(struct device *dev,
	struct device_attribute *attr, char *buf)
{
	pr_info("[PDBG] ois_hall_position_show sysfs");
	int rc = 0, i = 0;
    uint32_t cnt = 0;
...
static ssize_t ois_set_mode_store(struct device *dev,
	struct device_attribute *attr, const char *buf, size_t size)
{
	pr_info("[PDBG] ois_set_mode_store sysfs");
	int rc = 0;
	uint32_t mode = 0;
...
static ssize_t ois_gain_rear_show(struct device *dev,
	struct device_attribute *attr, char *buf)
{
	pr_info("[PDBG] ois_gain_rear_show sysfs");
	int rc = 0;
...
static ssize_t ois_check_cross_talk_show(struct device *dev,
	struct device_attribute *attr, char *buf)
{
	pr_info("[PDBG] ois_check_cross_talk_show sysfs");
	int rc = 0;
...
static ssize_t check_ois_valid_show(struct device *dev,
	struct device_attribute *attr, char *buf)
{
	pr_info("[PDBG] check_ois_valid_show sysfs");
	int rc = 0;
...
static ssize_t ois_check_hall_cal_show(struct device *dev,
	struct device_attribute *attr, char *buf)
{
	pr_info("[PDBG] ois_check_hall_cal_show sysfs");
	int rc = 0;
...
static ssize_t ois_ext_clk_show(struct device *dev,
	struct device_attribute *attr, char *buf)
{
	pr_info("[PDBG] ois_ext_clk_show sysfs");
	int rc = 0;
	uint32_t clk = 0;
...
static ssize_t ois_ext_clk_store(struct device *dev,
	struct device_attribute *attr, const char *buf, size_t size)
{
	pr_info("[PDBG] ois_ext_clk_store sysfs");
	int rc = 0;
	uint32_t clk = 0;
