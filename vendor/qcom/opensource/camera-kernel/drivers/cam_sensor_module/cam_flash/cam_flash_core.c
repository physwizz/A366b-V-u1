...
#if IS_REACHABLE(CONFIG_LEDS_S2MF301_FLASH)
...
int cam_flash_off(struct cam_flash_ctrl *flash_ctrl)
{
	...
	rc = s2mf301_fled_set_mode_ctrl(1, CAM_FLASH_MODE_OFF);
	if (rc < 0) {
		CAM_ERR(CAM_FLASH, "flash off fail");
		return -EINVAL;
	}
	...
	struct cam_flash_frame_setting *flash_data)
{
	...
	CAM_INFO(CAM_FLASH, "CAM Low Flash ON, %d mA", flash_data->led_current_ma[0]);
	rc = s2mf301_fled_set_curr(1, CAM_FLASH_MODE_PREFLASH, flash_data->led_current_ma[0]);
	if (rc < 0) {
		CAM_ERR(CAM_FLASH, "pre-flash set current fail");
		rc = -EINVAL;
	}
	rc |= s2mf301_fled_set_mode_ctrl(1, CAM_FLASH_MODE_PREFLASH);
	if (rc < 0) {
		CAM_ERR(CAM_FLASH, "pre-flash on fail");
		rc = -EINVAL;
	}
	...
	struct cam_flash_frame_setting *flash_data)
{
	...
	CAM_INFO(CAM_FLASH, "CAM Flash ON, %d mA", flash_data->led_current_ma[0]);
	rc = s2mf301_fled_set_curr(1, CAM_FLASH_MODE_FLASH, flash_data->led_current_ma[0]);
	if (rc < 0) {
		CAM_ERR(CAM_FLASH, "capture flash set current fail");
		rc = -EINVAL;
	}
	rc |= s2mf301_fled_set_mode_ctrl(1, CAM_FLASH_MODE_FLASH);
	if (rc < 0) {
		CAM_ERR(CAM_FLASH, "capture flash on fail");
		rc = -EINVAL;
	}
	...
	struct cam_flash_frame_setting *flash_data)
{
	...
	rc = s2mf301_fled_set_curr(1, CAM_FLASH_MODE_TORCH, flash_data->led_current_ma[0]);
	if (rc < 0) {
		CAM_ERR(CAM_FLASH, "torch flash set current fail");
		rc = -EINVAL;
	}
	rc |= s2mf301_fled_set_mode_ctrl(1, CAM_FLASH_MODE_TORCH);
	if (rc < 0) {
		CAM_ERR(CAM_FLASH, "torch flash on fail");
		rc = -EINVAL;
	}
	...
	if (req_id == 0) {
		...
		if ((flash_data->opcode == CAMERA_SENSOR_FLASH_OP_FIREHIGH) &&
			...
			if (flash_data->ispreflashoff == FALSE) {
				if (fctrl->is_fastcharger_disabled) {
					pdo_ctrl_by_flash(0);
					muic_afc_request_voltage(FLED, 9);
					fctrl->is_fastcharger_disabled = false;
					CAM_DBG(CAM_FLASH, "S2MF301 Enable fast charger");
				}
			}
#endif
			if (rc) {
	...
	case CAM_FLASH_PACKET_OPCODE_INIT: {
		...
		case CAMERA_SENSOR_FLASH_CMD_TYPE_INIT_FIRE: {
            ...
            if ((flash_operation_info->opcode == CAMERA_SENSOR_FLASH_OP_FIRELOW) || (flash_operation_info->opcode == CAMERA_SENSOR_FLASH_OP_FIRETORCH)) {
                ...
                fctrl->is_fastcharger_disabled = true;
                CAM_DBG(CAM_FLASH, "S2MF301 disable fast charger");
            }
#endif
