...
/*
adb shell "echo imx882 > /sys/module/camera/parameters/debug_sensor_name"
adb shell "echo 3072 > /sys/module/camera/parameters/i2c_debug_cnt"
*/
	...
	struct cam_sensor_ctrl_t *s_ctrl)
{
	...
	CAM_INFO(CAM_SENSOR, "cam_check_stream_on: Adaptive MIPI");
	...
	CAM_INFO(CAM_SENSOR, "sensor_id: %d", sensor_id);
	switch (sensor_id) {
		case SENSOR_ID_GC05A3:
		case SENSOR_ID_GC08A3:
		case SENSOR_ID_GC12A2:
		case SENSOR_ID_IMX882:
			ret = 1;
 			break;
		default:
			ret = 0;
...
int cam_sensor_apply_adaptive_mipi_settings(struct cam_sensor_ctrl_t *s_ctrl)
{
	...
	CAM_INFO(CAM_SENSOR, "cam_sensor_apply_adaptive_mipi_settings: Adaptive MIPI");
	...
	CAM_INFO(CAM_SENSOR, "cam_sensor_apply_adaptive_mipi_settings: check stream on");
	if (cam_check_stream_on(s_ctrl)
	...
	CAM_INFO(CAM_SENSOR, "cam_sensor_apply_adaptive_mipi_settings: DONE!");
	return rc;
}
#endif
	...
	bool onoff)
{
	...
	if ((onoff == false) && (s_ctrl->sensordata->slave_info.sensor_id == SENSOR_ID_GC05A3))
		FRAME_COUNT_REG_ADDR = 0x0117;
	else if ((onoff == false) && (s_ctrl->sensordata->slave_info.sensor_id == SENSOR_ID_IMX882))
		FRAME_COUNT_REG_ADDR = 0x0005;
	else
		return rc;
	...
	for (i = 0; i < retries; i++) {
		...
		if ((onoff == false) && (s_ctrl->sensordata->slave_info.sensor_id == SENSOR_ID_GC05A3) && (reg_data & 0x90)  == 0x00) {
			CAM_INFO(CAM_SENSOR, "[CNT_DBG] sensor 0x%x : Stream off, Last frame_cnt 0x%x",
				s_ctrl->sensordata->slave_info.sensor_id, reg_data);
			rc = 0;
			break;
		}
		else if (((onoff == true) && (reg_data != 0xff)) ||
	...
	void *arg)
{
	...
	case CAM_SENSOR_PROBE_CMD: {
		...
		if (((rc == -ENODEV) || (rc == -ENOTCONN) || (rc == -EINVAL)) &&
			((s_ctrl->soc_info.index == 0) && (s_ctrl->sensordata->slave_info.sensor_id == 0x8e8))) //s5kgn8 sensor
		{
			CAM_ERR(CAM_SENSOR,
