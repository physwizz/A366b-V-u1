...
uint64_t camera_op_clk[6];

void set_cam_op_clk(int csiphy_num, int cphy_mode, uint64_t data_rate);
uint64_t get_cam_op_clk(int index);
#endif
			...
			void *arg)
{
	...
	case CAM_START_DEV: {
		...
		CAM_INFO(CAM_CSIPHY,
			"DBG: %d, Type: %s, Datarate: %llu, data_rate_variant_idx: %d", soc_info->index,
			g_phy_data[soc_info->index].is_3phase ? "CPHY" : "DPHY",
			csiphy_dev->csiphy_info[offset].data_rate, data_rate_variant_idx);
		...
		if (cam_clock_data_recovery_is_requested()) {
			cam_clock_data_recovery_write_register(csiphybase, soc_info->index);
		...
		set_mipi_info(soc_info->index,
			g_phy_data[soc_info->index].is_3phase,
			csiphy_dev->csiphy_info[offset].data_rate);
#endif
#if defined(CONFIG_SAMSUNG_CAMERA)
		set_cam_op_clk(soc_info->index, g_phy_data[soc_info->index].is_3phase, csiphy_dev->csiphy_info[offset].data_rate);
#endif
		CAM_INFO(CAM_CSIPHY,
...
void set_cam_op_clk(int csiphy_num, int cphy_mode, uint64_t data_rate)
{
	if (cphy_mode == 1)
	{
		camera_op_clk[csiphy_num] = data_rate * 3 /10;
	}
	else if (cphy_mode == 0)
	{
		camera_op_clk[csiphy_num] = data_rate * 4 /10;
	}
	else
		CAM_ERR(CAM_CSIPHY, "Invalid cphy mode : %d", cphy_mode);
}

uint64_t get_cam_op_clk(int index)
{
	return camera_op_clk[index];
}
#endif
