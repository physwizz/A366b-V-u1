...
#if defined(CONFIG_SAMSUNG_REAR_MARCO)
char marco_module_fw_ver[FROM_MODULE_FW_INFO_SIZE+1];
#endif
...
#include "gc05a3_otp.h"
struct cam_sensor_i2c_reg_setting load_gc05a3_otp_setfile = {
	load_sensor_gc05a3_otp_setfile_reg,
	sizeof(load_sensor_gc05a3_otp_setfile_reg)/sizeof(load_sensor_gc05a3_otp_setfile_reg[0]),
	CAMERA_SENSOR_I2C_TYPE_WORD,
	CAMERA_SENSOR_I2C_TYPE_BYTE,
	50
};

struct cam_sensor_i2c_reg_setting gc05a3_otp_init_read_setting = {
	gc05a3_otp_init_read_reg,
	sizeof(gc05a3_otp_init_read_reg)/sizeof(gc05a3_otp_init_read_reg[0]),
	CAMERA_SENSOR_I2C_TYPE_WORD,
	CAMERA_SENSOR_I2C_TYPE_BYTE,
	10
};

struct cam_sensor_i2c_reg_setting gc05a3_otp_init_write_setting = {
	gc05a3_otp_init_write_reg,
	sizeof(gc05a3_otp_init_write_reg)/sizeof(gc05a3_otp_init_write_reg[0]),
	CAMERA_SENSOR_I2C_TYPE_WORD,
	CAMERA_SENSOR_I2C_TYPE_BYTE,
	10
};

struct cam_sensor_i2c_reg_setting gc05a3_otp_finish_read_setting = {
	gc05a3_otp_finish_read_reg,
	sizeof(gc05a3_otp_finish_read_reg)/sizeof(gc05a3_otp_finish_read_reg[0]),
	CAMERA_SENSOR_I2C_TYPE_WORD,
	CAMERA_SENSOR_I2C_TYPE_BYTE,
	10
};

struct cam_sensor_i2c_reg_setting gc05a3_otp_finish_write_setting = {
	gc05a3_otp_finish_write_reg,
	sizeof(gc05a3_otp_finish_write_reg)/sizeof(gc05a3_otp_finish_write_reg[0]),
	CAMERA_SENSOR_I2C_TYPE_WORD,
	CAMERA_SENSOR_I2C_TYPE_BYTE,
	10
};
...
struct cam_sensor_i2c_reg_setting load_gc08a3_otp_setfile = {
	load_sensor_gc08a3_otp_setfile_reg,
	sizeof(load_sensor_gc08a3_otp_setfile_reg)/sizeof(load_sensor_gc08a3_otp_setfile_reg[0]),
	CAMERA_SENSOR_I2C_TYPE_WORD,
	CAMERA_SENSOR_I2C_TYPE_BYTE,
	50
};

struct cam_sensor_i2c_reg_setting gc08a3_otp_init_read_setting = {
	gc08a3_otp_init_read_reg,
	sizeof(gc08a3_otp_init_read_reg)/sizeof(gc08a3_otp_init_read_reg[0]),
	CAMERA_SENSOR_I2C_TYPE_WORD,
	CAMERA_SENSOR_I2C_TYPE_BYTE,
	10
};

struct cam_sensor_i2c_reg_setting gc08a3_otp_init_write_setting = {
	gc08a3_otp_init_write_reg,
	sizeof(gc08a3_otp_init_write_reg)/sizeof(gc08a3_otp_init_write_reg[0]),
	CAMERA_SENSOR_I2C_TYPE_WORD,
	CAMERA_SENSOR_I2C_TYPE_BYTE,
	10
};

struct cam_sensor_i2c_reg_setting gc08a3_otp_finish_read_setting = {
	gc08a3_otp_finish_read_reg,
	sizeof(gc08a3_otp_finish_read_reg)/sizeof(gc08a3_otp_finish_read_reg[0]),
	CAMERA_SENSOR_I2C_TYPE_WORD,
	CAMERA_SENSOR_I2C_TYPE_BYTE,
	10
};

struct cam_sensor_i2c_reg_setting gc08a3_otp_finish_write_setting = {
	gc08a3_otp_finish_write_reg,
	sizeof(gc08a3_otp_finish_write_reg)/sizeof(gc08a3_otp_finish_write_reg[0]),
	CAMERA_SENSOR_I2C_TYPE_WORD,
	CAMERA_SENSOR_I2C_TYPE_BYTE,
	10
};
...
#if defined(CONFIG_GC05A3_OTP)
/**
  * cam_otp_gc05a3_read() - read map data into buffer for gc02m1
  ...
  */
static int cam_otp_gc05a3_read(struct cam_eeprom_ctrl_t *e_ctrl, uint32_t addr,
                                uint8_t *memptr)
{
    int rc = 0;
...
/**
  * cam_otp_gc05a3_read_memory() - read map data into buffer
  * @e_ctrl:    otp control struct
                                      ...
                                      struct cam_eeprom_memory_block_t *block)
{
		...
		CAM_INFO(CAM_EEPROM,"load_otp_setfile=%d index=%d",
			sizeof(load_gc05a3_otp_setfile)/sizeof(struct cam_sensor_i2c_reg_array), e_ctrl->soc_info.index);
	
		//load otp global setfile
		rc = camera_io_dev_write(&e_ctrl->io_master_info, &load_gc05a3_otp_setfile);
		if (rc < 0)
		{
			CAM_ERR(CAM_EEPROM,"%s:(%d) load otp globle setfile failed\n", __func__, __LINE__);
			return rc;
		}
	
		//load read init setfile
		rc = camera_io_dev_write(&e_ctrl->io_master_info, &gc05a3_otp_init_read_setting);
		if (rc < 0)
		{
		...
		// select bank
		rc = cam_otp_gc05a3_read(e_ctrl, GC05A3_SENSOR_OTP_BANK_SELECT_REGISTER, &OTP_Bank);
		if (rc < 0)
		{
			CAM_ERR(CAM_EEPROM,"%s:(%d) read failed\n", __func__, __LINE__);
		}
		CAM_ERR(CAM_EEPROM,"%s:%d read OTP_Bank: %d\n", __func__, __LINE__, OTP_Bank);
		...
		case 0: //BANK1
		case 0x1:
			offset = GC05A3_SENSOR_OTP_BANK1_START_REGISTER;
			break;
	
		case 0x3: //BANK2
			offset = GC05A3_SENSOR_OTP_BANK2_START_REGISTER;
			break;
	
		case 0x7:
			offset = GC05A3_SENSOR_OTP_BANK3_START_REGISTER;
			break;
	
		default:
			offset = GC05A3_SENSOR_OTP_BANK1_START_REGISTER;
			CAM_ERR(CAM_EEPROM,"%s: Bank error : Bank(%d)\n", __func__, OTP_Bank);
			break;
		}
	
		CAM_ERR(CAM_EEPROM,"%s:%d read OTP offset: 0x%x\n", __func__, __LINE__, offset);
		...
		for (j = 1; j < block->num_map; j++)
		{
			...
			while(read_size > 0)
			{
				rc = cam_otp_gc05a3_read(e_ctrl, addr, memptr);
				if (rc < 0)
				{
...
#endif //CONFIG_GC05A3_OTP

#if defined(CONFIG_GC08A3_OTP)


/**
  * cam_otp_gc08a3_write() - read map data into buffer for gc02m1
  * @e_ctrl:    otp control struct
  * @addr:    memory address to be write
  * @data:    memory data to be write
  *
  * This function iterates through blocks stored in block->map, reads each
  * region and concatenate them into the pre-allocated block->mapdata
  */
static int cam_otp_gc08a3_write(struct cam_eeprom_ctrl_t *e_ctrl, uint32_t addr, uint32_t data)
{
    int rc = 0;
    struct cam_sensor_i2c_reg_setting i2c_reg_settings;
    struct cam_sensor_i2c_reg_array i2c_reg_array;
...
static int cam_otp_gc08a3_read(struct cam_eeprom_ctrl_t *e_ctrl, uint16_t addr,
                                uint8_t *memptr)
{
    int rc = 0;
    enum camera_sensor_i2c_type data_type = CAMERA_SENSOR_I2C_TYPE_BYTE;
    ...
    // read one byte
    rc = camera_io_dev_read_seq(&e_ctrl->io_master_info, GC08A3_OTP_READ_ADDR, memptr, addr_type, data_type, 1);
    if (rc < 0)
    {
        CAM_ERR(CAM_EEPROM, "read failed rc %d", rc);
    }
	CAM_DBG(CAM_EEPROM,"addr = 0x%02X memptr[0]=%c *memptr=%c", addr, memptr[0], *memptr);
                                      ...
                                      struct cam_eeprom_memory_block_t *block)
{
	...
	struct cam_eeprom_memory_map_t *emap = block->map;
    uint8_t *memptr = block->mapdata;
    uint16_t addr = 0, read_size = 0;
	int j = 0;
    uint8_t OTP_Bank = 0;
	uint16_t start_addr = 0;
	uint8_t start_addr_h = 0;
	uint8_t start_addr_l = 0;
    ...
    CAM_INFO(CAM_EEPROM,"load_otp_setfile=%d index=%d",
        sizeof(load_gc08a3_otp_setfile)/sizeof(struct cam_sensor_i2c_reg_array), e_ctrl->soc_info.index);
#if 0

    /* 1. prepare to otp read : sensor is not initialized */
    i2c_reg_array.reg_addr = 0x031C;
    i2c_reg_array.reg_data = 0x60;
    i2c_reg_settings.reg_setting = &i2c_reg_array;
    //CAM_INFO(CAM_EEPROM,"low addr = 0x%02X ", i2c_reg_array.reg_data);
    ...
    i2c_reg_array.reg_addr = 0x0315;
    i2c_reg_array.reg_data = 0x80;
    i2c_reg_settings.reg_setting = &i2c_reg_array;
    //CAM_INFO(CAM_EEPROM,"low addr = 0x%02X ", i2c_reg_array.reg_data);
    ...
    //load otp global setfile
    rc = camera_io_dev_write(&e_ctrl->io_master_info, &load_gc08a3_otp_setfile);
    if (rc < 0)
    {
        pr_err("%s:(%d) load otp globle setfile failed\n", __func__, __LINE__);
        return rc;
    }
    usleep_range(2000, 2100); /* 10ms delay */

    //load read init setfile
    rc = camera_io_dev_write(&e_ctrl->io_master_info, &gc08a3_otp_init_read_setting);
    if (rc < 0)
    {
        CAM_ERR(CAM_EEPROM,"load read init setfile failed");
        return rc;
    ...
    // select bank
    rc = cam_otp_gc08a3_write(e_ctrl, GC08A3_OTP_ACCESS_ADDR_HIGH,((GC08A3_BANK_SELECT_ADDR >> 8) & 0xFF));
    rc = cam_otp_gc08a3_write(e_ctrl, GC08A3_OTP_ACCESS_ADDR_LOW,(GC08A3_BANK_SELECT_ADDR & 0xFF));
    rc = cam_otp_gc08a3_write(e_ctrl, GC08A3_OTP_MODE_SEL_ADDR, GC08A3_READ_MODE);
    rc = cam_otp_gc08a3_read(e_ctrl, GC08A3_OTP_READ_ADDR, &OTP_Bank);
    if (rc < 0)
    {
        CAM_ERR(CAM_EEPROM,"%s:(%d) read failed\n", __func__, __LINE__);
    }
    ...
    rc = cam_otp_gc08a3_write(e_ctrl, GC08A3_OTP_ACCESS_ADDR_HIGH,start_addr_h);
    rc = cam_otp_gc08a3_write(e_ctrl, GC08A3_OTP_ACCESS_ADDR_LOW,start_addr_l);
    rc = cam_otp_gc08a3_write(e_ctrl, GC08A3_OTP_MODE_SEL_ADDR, GC08A3_READ_MODE);
    rc = cam_otp_gc08a3_write(e_ctrl, GC08A3_OTP_MODE_SEL_ADDR, GC08A3_OTP_ACC);
	...
	for (index = 0; index < GC08A3_OTP_USED_CAL_SIZE; index++) {
		rc = cam_otp_gc08a3_read(e_ctrl, &memptr[index]);
		//CAM_INFO(CAM_EEPROM,"addr = 0x%02X memptr[0]=%c *memptr=0x%x (%c)", index, memptr[0], memptr[index], memptr[index]);
		if (rc < 0) {
			CAM_ERR(CAM_EEPROM,"failed to otp_read(index:%d, ret:%d)", index, rc);
			goto err;
		}
	}
#endif
    ...
    memptr = block->mapdata;
    CAM_ERR(CAM_EEPROM,"read data done memptr=%pK VR:: End",memptr);

err:
    CAM_ERR(CAM_EEPROM,"OTP off");	
	cam_otp_gc08a3_write(e_ctrl, 0x0ACE, 0x00);
	cam_otp_gc08a3_write(e_ctrl, GC08A3_OTP_MODE_SEL_ADDR, 0x00);
	cam_otp_gc08a3_write(e_ctrl, 0x0A67, 0x00);
	cam_otp_gc08a3_write(e_ctrl, 0x0316, 0x00);

	cam_otp_gc08a3_write(e_ctrl, 0x0315, 0x00);
	cam_otp_gc08a3_write(e_ctrl, 0x031C, 0x00);
	usleep_range(2000, 2100); /* 2ms delay */

    return rc;
}
									  
#endif //CONFIG_GC08A3_OTP
...
#if defined(CONFIG_SEC_A36XQ_PROJECT)
