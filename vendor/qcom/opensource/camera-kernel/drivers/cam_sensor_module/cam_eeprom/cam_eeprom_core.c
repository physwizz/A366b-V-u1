...
eeropm_crc_check :
		...
		if (e_ctrl->soc_info.index == SEC_ULTRA_WIDE_SENSOR){
		    CAM_ERR(CAM_EEPROM, "GC08A3 REAR UW");
		    rc = cam_otp_gc08a3_read_memory(e_ctrl, &e_ctrl->cal_data);
		}
		else if (e_ctrl->soc_info.index == SEC_MACRO_SENSOR){
			CAM_ERR(CAM_EEPROM, "GC05A3 REAR MARCO");
			rc = cam_otp_gc05a3_read_memory(e_ctrl, &e_ctrl->cal_data);
		}	
		else {
		    rc = cam_eeprom_read_memory(e_ctrl, &e_ctrl->cal_data);
		}
